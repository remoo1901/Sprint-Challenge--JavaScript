...
for(let i = 0 ; i < graduates.length ; i++){
  if (graduates[i].university.includes("university")){
...
return  displayNames.push(`Name: ${x.animal_name}, Scientific: ${x.scientific_name}`);
});
/*const items = [1, 29, 47];
const copy = [];
...
zooAnimals.map((i) =>{
  lowCaseAnimalNames.push(`${i.animal_name}`)
...
The zoos are concerned about animals with a lower population count. Using filter, create a new array of objects called lowPopulationAnimals which contains only the animals with 
